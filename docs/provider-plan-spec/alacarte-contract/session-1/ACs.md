...
| AC-TST-4 | D8 | `/test-engineer` | `ALACARTE_SCENARIO_KEYS` shape: length 2, both keys (`TICKETS_5`, `SESSIONS_2`) present | Test run: `npm run build && npm test` | `ALACARTE_SCENARIO_KEYS.length === 2` and both key strings are present in the array | PASS · "ALACARTE_SCENARIO_KEYS has length 2 and contains both scenario keys" ✔ | PASS · confirmed. Note: `CheckoutRequest` has no Zod schema by design (TS-only discriminated union, spec D2); discrimination is proven by build clean, not a runtime test. | |
